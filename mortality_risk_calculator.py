...
if page == "home":
    ...
    # Hero — CSS grid collapses to single column on mobile automatically
    st.markdown(f"""
    <style>
    .home-grid {{
      display:grid;
    ...
    <div class="home-grid">
      <div class="home-hero">
        <div style="font-family:'Space Grotesk',sans-serif;font-size:13px;font-weight:700;
                    letter-spacing:0.16em;text-transform:uppercase;color:{T["teal"]};margin-bottom:14px;">
        ...
        <p style="font-size:16px;color:{T["text_secondary"]};line-height:1.80;
        ...
        <div class="ref-row">
          ...
          <div>
            <div style="font-family:\'Space Grotesk\',sans-serif;font-size:15px;
              ...
              {rl} <span style="color:{T["text_secondary"]};font-weight:400;font-size:13px;">({cfg["band"]})</span>
            </div>
            <div style="font-size:13px;color:{T["text_secondary"]};margin-top:3px;line-height:1.5;">
              {cfg["rec"][:90]}...
            </div>
          </div>
        </div>""" for rl, cfg in RISK_CFG.items()])}
      </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    # Stats — CSS grid, 4 cols desktop / 2 cols mobile
    st.markdown(f"""
    <div class="home-stats">
      {''.join([f"""<div class="icu-stat">
