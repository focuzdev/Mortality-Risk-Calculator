...
.theme-toggle-wrap {{
  ...
  transition:background 0.2s,box-shadow 0.2s;user-select:none;flex-shrink:0;
}}
.theme-toggle-wrap:hover {{
  background:{t["teal_glow"]};box-shadow:0 0 0 3px {t["teal_glow"]};
}}
.theme-track {{
  width:42px;height:22px;border-radius:999px;
  background:{_pill_track};position:relative;
  transition:background 0.3s;flex-shrink:0;
}}
.theme-knob {{
  position:absolute;top:2px;left:0;
  width:18px;height:18px;border-radius:50%;
  background:{_pill_knob_bg};
  box-shadow:0 1px 4px rgba(0,0,0,0.35);
  transition:transform 0.28s cubic-bezier(.34,1.56,.64,1);
}}

/* ── Hide the hidden theme st.button only — all others visible normally ── */
div[data-testid="stButton"]:has(button#icu-theme-st-btn) {{
...
# Pill toggle colours derived from active theme
_pill_track   = T["teal"] if is_dark else T["border"]
_pill_knob_bg = "#05101f" if is_dark else "#ffffff"
_pill_knob_x  = "26px"   if is_dark else "2px"
_sun_op       = "1"       if is_dark else "0.35"
_moon_op      = "0.35"    if is_dark else "1"
...
<div class="icu-nav">
  ...
  <div class="icu-nav-right">
    ...
    <!-- Advanced pill toggle — clicking fires the single hidden st.button -->
    <div class="theme-toggle-wrap" id="theme-pill-btn"
         title="{'Switch to Light Mode' if is_dark else 'Switch to Dark Mode'}"
         onclick="(function(){{
           var b=window.parent.document.getElementById('icu-theme-st-btn');
           if(b){{b.click();}}
         }})()">
      <!-- Moon icon -->
      <svg width="14" height="14" viewBox="0 0 24 24" fill="none"
           style="opacity:{_moon_op};transition:opacity 0.3s;flex-shrink:0;">
        <path d="M21 12.79A9 9 0 1 1 11.21 3a7 7 0 0 0 9.79 9.79z"
              fill="{T['teal']}" />
      </svg>
      <!-- Track -->
      <div class="theme-track">
        <div class="theme-knob" style="transform:translateX({_pill_knob_x});"></div>
      </div>
      <!-- Sun icon -->
      <svg width="15" height="15" viewBox="0 0 24 24" fill="none"
           style="opacity:{_sun_op};transition:opacity 0.3s;flex-shrink:0;">
        <circle cx="12" cy="12" r="5" fill="{T['teal']}"/>
        <line x1="12" y1="1"  x2="12" y2="3"  stroke="{T['teal']}" stroke-width="2" stroke-linecap="round"/>
        <line x1="12" y1="21" x2="12" y2="23" stroke="{T['teal']}" stroke-width="2" stroke-linecap="round"/>
        <line x1="4.22"  y1="4.22"  x2="5.64"  y2="5.64"  stroke="{T['teal']}" stroke-width="2" stroke-linecap="round"/>
        <line x1="18.36" y1="18.36" x2="19.78" y2="19.78" stroke="{T['teal']}" stroke-width="2" stroke-linecap="round"/>
        <line x1="1"  y1="12" x2="3"  y2="12" stroke="{T['teal']}" stroke-width="2" stroke-linecap="round"/>
        <line x1="21" y1="12" x2="23" y2="12" stroke="{T['teal']}" stroke-width="2" stroke-linecap="round"/>
        <line x1="4.22"  y1="19.78" x2="5.64"  y2="18.36" stroke="{T['teal']}" stroke-width="2" stroke-linecap="round"/>
        <line x1="18.36" y1="5.64"  x2="19.78" y2="4.22"  stroke="{T['teal']}" stroke-width="2" stroke-linecap="round"/>
      </svg>
    </div>
  </div>
</div>
...
# Single hidden Streamlit button — given a unique DOM id via JS below
if st.button("__theme__", key="theme_btn"):
    st.session_state.dark_mode = not st.session_state.dark_mode
    st.rerun()

# Assign the unique id so the inline onclick can find it reliably
st.markdown("""
<script>
(function stamp(){
  ...
  btns.forEach(function(b){
    if(b.innerText && b.innerText.trim()==='__theme__'){
      b.id = 'icu-theme-st-btn';
      b.style.cssText='position:fixed;opacity:0;width:0;height:0;pointer-events:auto;';
      found = true;
    }
  });
  if(!found){ setTimeout(stamp, 120); }
})();
</script>
""", unsafe_allow_html=True)
              ...
              <div style="margin-top:18px;padding:16px 18px;background:{rc['bg']};
                ...
                <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;">
                  ...
                  <div style="font-family:'Space Grotesk',sans-serif;font-size:15px;
                    ...
                    {{"LOW":"Favourable Prognosis","MODERATE":"Cautious Prognosis",
                      "HIGH":"Guarded Prognosis","VERY HIGH":"Poor Prognosis"}}.get("{rl}","")
                  </div>
                  <!-- risk tier pill -->
