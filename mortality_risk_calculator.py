...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with tab1:
        ...
        <div style="background:{T["warn_bg"]};border:1px solid {T["warn_border"]};
          ...
          ℹ️ Complete this tab, then proceed to <strong>COMORBIDITIES (CCI)</strong>
          before opening <strong>RESULTS</strong>.
        </div>""", unsafe_allow_html=True)
    ...
    with tab2:
        ...
        <div style="display:flex;gap:14px;margin-bottom:22px;flex-wrap:wrap;">
          ...
          <div class="cci-badge"><div class="cb-lbl">Condition Points</div>
            <div class="cb-val" style="color:{T["feat3"]};">{cp2} pts</div><div class="cb-sub">Tick below</div></div>
          <div class="cci-badge"><div class="cb-lbl">Total CCI Score</div>
            <div class="cb-val" style="color:{T["teal"]};">{ct2}</div><div class="cb-sub">Used in model</div></div>
        </div>""", unsafe_allow_html=True)

        st.markdown(f'<div class="sec-ttl">🩺 Tick All Applicable Comorbid Conditions</div>', unsafe_allow_html=True)
        ...
        ct3  = ca2 + cp3
        st.markdown(f"""
        <div style="margin-top:20px;background:{T["chip_bg"]};border:1px solid {T["teal"]};
          ...
          <span style="font-size:11px;color:{T["text_secondary"]};margin-left:auto;">
            Note: CCI=0 uses log(0.5) floor to avoid model singularity</span>
        </div>""", unsafe_allow_html=True)
    ...
    with tab3:
        ...
        if ca_r  == "Not selected":          errs.append("Cardiac arrest")
        ...
        if errs:
            ...
            <div style="background:{T["err_bg"]};border:1px solid {T["err_border"]};
              ...
              <div style="font-size:13px;color:{T["text_secondary"]};line-height:1.8;">
                Please complete the following in <strong style="color:{T["text_primary"]};">Patient Details</strong>:<br>
        ...
        <div class="risk-banner" style="background:{rc["bg"]};border-color:{rc["border"]};color:{rc["color"]};">
          <div style="display:flex;align-items:center;gap:18px;flex-wrap:wrap;">
            ...
            <div style="text-align:right;flex-shrink:0;">
              <div style="font-size:10px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;opacity:0.7;">30-Day Estimate</div>
        ...
        for col, (lbl, val, ci, clr, sub, ci_lbl) in zip(mc, cards):
            with col:
                st.markdown(f"""
        ...
        with cha:
            st.markdown(f'''<div style="font-family:'Space Grotesk',sans-serif;font-size:13px;font-weight:700;
                color:{T["text_primary"]};margin-bottom:10px;">📈 Mortality Risk Curve with 95% CI — Days 1–30</div>''',
                unsafe_allow_html=True)
            ...
            fig.add_trace(go.Scatter(
                ...
                y=dy_hi + dy_lo[::-1],
                fill="toself",
                fillcolor=rc["bg"].replace("0.09", "0.20"),
            ...
            fig.update_layout(
                ...
                yaxis=dict(title="Mortality Risk (%)", color=pf, gridcolor=pg, range=[0, max(max(dy_hi) * 1.25, 5)]),
        ...
        with chb:
            st.markdown(f'''<div style="font-family:'Space Grotesk',sans-serif;font-size:13px;font-weight:700;
                color:{T["text_primary"]};margin-bottom:10px;">🧩 Factor Score Contributions</div>''',
                unsafe_allow_html=True)
            sb = pd.DataFrame([
        ...
        st.markdown(f'''<div style="font-family:'Space Grotesk',sans-serif;font-size:13px;font-weight:700;
            color:{T["text_primary"]};margin:4px 0 12px 0;">🔬 Cox PH Score Breakdown</div>''',
            unsafe_allow_html=True)
        bl   = list(BETA.values())
        bl_se = list(BETA_SE.values())
        rows = [{"Factor": k, "β": f"{bl[i]:+.5f}", "SE(β)": f"±{bl_se[i]:.4f}", "Contribution": f"{v:+.6f}"}
                for i, (k, v) in enumerate(bd.items())]
        rows.append({"Factor": "▶  COMPOSITE SCORE (Σ)", "β": "—", "SE(β)": f"±{se_comp:.4f}", "Contribution": f"{comp:+.6f}"})
        st.dataframe(pd.DataFrame(rows), hide_index=True, use_container_width=True)

        # ── Platelet contextual flag ──
        ...
        st.markdown("<br>", unsafe_allow_html=True)
        if st.button("🔄  New Patient", type="secondary", key="new_pt"):
            keep = {"page", "dark_mode"}
        ...
        <div style="margin-top:22px;background:{T["warn_bg"]};border:1px solid {T["warn_border"]};
          ...
          This tool is a decision-support aid for trained clinical professionals only. It does not replace
          multidisciplinary team judgement or individualised patient assessment. All outputs are probabilistic
          estimates with inherent uncertainty (shown as 95% CI). <strong>Version 2.0 — September 2025.</strong>
        </div>""", unsafe_allow_html=True)
