...
with nl:
    st.markdown(f"""<div style="display:flex;align-items:center;gap:12px;padding:14px 0 10px 0;">
      <div class="icu-logo-ring">
        <svg width="22" height="22" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M12 21C12 21 3 14.5 3 8.5C3 5.42 5.42 3 8.5 3C10.24 3 11.81 3.81 12 4C12.19 3.81 13.76 3 15.5 3C18.58 3 21 5.42 21 8.5C21 14.5 12 21 12 21Z" fill="white" stroke="none"/>
          <polyline points="2,9 6,9 8,5 10,13 12,7 14,11 16,9 22,9" stroke="rgba(5,16,31,0.7)" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" fill="none"/>
        </svg>
      </div>
...
with nr:
    ...
    pill_bg  = "#0a1f38" if is_dark else "#e8f4f0"
    pill_bdr = "#00c9b1" if is_dark else "#00897b"
    thumb_bg = "linear-gradient(135deg,#00c9b1,#0096d6)" if is_dark else "linear-gradient(135deg,#00897b,#0277bd)"
    thumb_l  = "calc(100% - 27px)" if is_dark else "3px"
    lbl_clr  = "#8fa3b8" if is_dark else "#4d6a87"
    t_icon   = "&#127769;" if is_dark else "&#9728;&#65039;"   # moon / sun (HTML entities)
    lbl_text = "Dark Mode" if is_dark else "Light Mode"

    # Single-element toggle: a styled <form> whose submit button IS the pill
    st.markdown(f"""
    <style>
    /* ── Theme toggle pill form ── */
    #theme-form {{ margin:0; padding:0; border:none; }}
    ...
    #theme-form button[type="submit"]:focus {{ outline:none !important; }}
    .t-lbl {{
    ...
    .t-pill {{
        ...
        background:{pill_bg}; border:1.5px solid {pill_bdr};
    ...
    .t-thumb {{
        ...
        background:{thumb_bg};
        position:absolute; top:2px; left:{thumb_l};
        display:flex; align-items:center; justify-content:center;
        font-size:13px; line-height:1;
        box-shadow:0 1px 5px rgba(0,0,0,0.28);
    }}
    </style>
    <form id="theme-form" method="get" action="">
      <button type="submit" name="_theme_toggle" value="1">
        <span class="t-lbl">{lbl_text}</span>
        <div class="t-pill"><div class="t-thumb">{t_icon}</div></div>
      </button>
    </form>
    """, unsafe_allow_html=True)

    # Detect the form submission via query params
    qp = st.query_params
    if "_theme_toggle" in qp:
        qp.clear()
