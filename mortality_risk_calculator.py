...
with nr:
    ...
    lbl_color = "#8fa3b8" if is_dark else "#4d6a87"
    lbl_text  = "🌙  DARK" if is_dark else "☀️  LIGHT"
    trk_bg    = "#00c9b1" if is_dark else "#cbd5e1"
    thumb_grad= "linear-gradient(135deg,#00c9b1,#0096d6)" if is_dark else "linear-gradient(135deg,#00897b,#0277bd)"

    # Hide Streamlit's own label — we draw our own HTML label beside the pill
    st.markdown(f"""
    <style>
    /* Wrapper: push everything to the right, vertically centred */
    ...
    /* Hide Streamlit's built-in text label completely */
    div[data-testid="stToggle"] p {{
        display:none !important;
    }}
    /* Track */
    div[data-testid="stToggle"] [role="switch"] {{
        background:{trk_bg} !important;
    ...
    /* Our custom label injected via HTML before the toggle */
    .nav-toggle-label {{
    ...
    <div style="display:flex;align-items:center;justify-content:flex-end;padding-top:15px;gap:10px;">
      <span class="nav-toggle-label">{lbl_text}</span>
    </div>
    """, unsafe_allow_html=True)

    toggled = st.toggle(
        " ",                          # single space — label hidden by CSS above
        value=is_dark,
        key="theme_toggle_widget",
        label_visibility="collapsed", # removes label space entirely
    )
    if toggled != is_dark:
