...
@media(max-width:768px){{
  ...
  .icu-card{{padding:18px 16px !important;}}
  .icu-stat{{padding:16px 18px !important;}}
  .icu-stat .val{{font-size:26px !important;}}
  .risk-banner{{padding:18px 20px !important;}}
  ...
  .stepper-wrap{{padding:14px 16px !important;}}
  .step-label{{font-size:12px !important;}}
  div[data-testid="stHorizontalBlock"]{{flex-wrap:wrap !important;}}
  div[data-testid="stColumn"]{{min-width:140px !important;}}
}}
...
.stButton>button[kind="secondary"]{{
  ...
  color:{t["btn_text"]} !important;border:none !important;padding:14px 30px !important;
  box-shadow:0 4px 18px {t["shadow_btn"]} !important;}}
.stButton>button[kind="secondary"]:hover{{transform:translateY(-2px) !important;box-shadow:0 8px 26px {t["shadow_btn"]} !important;}}
.stButton>button[kind="secondary"] p{{color:inherit !important;margin:0 !important;}}

/* ── Theme toggle: scoped ONLY via title attribute — never bleeds to other buttons ── */
.stButton>button[title="Switch to Dark Mode"],
.stButton>button[title="Switch to Light Mode"]{{
  ...
  margin-left:auto !important;transition:all 0.2s ease !important;}}
.stButton>button[title="Switch to Dark Mode"]:hover,
.stButton>button[title="Switch to Light Mode"]:hover{{
...
div[data-testid="stColumn"]:last-child .stButton{{
  display:flex !important;justify-content:flex-end !important;padding-top:0 !important;margin-top:0 !important;}}

/* ── Button row: force equal height and perfect vertical alignment ── */
div[data-testid="stHorizontalBlock"]:has(button[kind="primary"]) {{
  align-items:stretch !important;
}}
div[data-testid="stHorizontalBlock"]:has(button[kind="primary"]) > div[data-testid="stColumn"] {{
...
div[data-testid="stHorizontalBlock"]:has(button[kind="primary"]) .stButton {{
  flex:1 !important;display:flex !important;flex-direction:column !important;padding:0 !important;margin:0 !important;
}}
...
.stDataFrame th{{background:{t["card_bg"]} !important;color:{t["text_label"]} !important;
  font-size:12px !important;letter-spacing:0.07em !important;text-transform:uppercase !important;}}
.stDataFrame td{{color:{t["text_primary"]} !important;font-size:13px !important;}}

/* ── Caption / small text ── */
.stCaption,.stCaption p{{color:{t["text_secondary"]} !important;font-size:13px !important;}}
hr{{border-color:{t["border"]} !important;margin:8px 0 18px 0 !important;}}
...
.icu-badge{{background:{t["chip_bg"]};border:1px solid {t["teal"]};border-radius:20px;padding:5px 14px;
  font-size:12px;font-family:"JetBrains Mono",monospace;color:{t["teal"]};letter-spacing:0.04em;
...
.icu-stat .lbl{{font-size:12px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;
  color:{t["text_secondary"]};margin-bottom:8px;}}
.icu-stat .val{{font-family:"Space Grotesk",sans-serif;font-size:32px;font-weight:700;
  color:{t["text_primary"]};line-height:1;}}
.icu-stat .sub{{font-size:13px;color:{t["text_secondary"]};margin-top:6px;}}
.sec-ttl{{font-family:"Space Grotesk",sans-serif;font-size:12px;font-weight:700;letter-spacing:0.09em;
  text-transform:uppercase;color:{t["teal"]};padding:10px 0 8px 0;
  border-bottom:1px solid {t["border"]};margin-bottom:16px;}}
.risk-banner{{border-radius:16px;padding:22px 26px;border:1px solid;position:relative;overflow:hidden;}}
...
/* Mortality chips — CSS grid, adapts to screen */
.mchip-grid{{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-bottom:16px;}}
.mchip{{border-radius:14px;padding:18px 20px;border:1px solid;position:relative;
  overflow:hidden;background:{t["stat_bg"]};min-width:0;}}
.mchip .ml{{font-size:11px;font-weight:700;letter-spacing:0.10em;text-transform:uppercase;
  margin-bottom:8px;color:{t["text_secondary"]};white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}}
.mchip .mv{{font-family:"Space Grotesk",sans-serif;font-size:26px;font-weight:700;line-height:1;}}
.mchip .ms{{font-size:13px;font-weight:600;margin-top:5px;opacity:0.85;}}
.mchip .ci{{font-size:11px;margin-top:3px;opacity:0.70;font-family:"JetBrains Mono",monospace;
  white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}}

.feat-chip{{background:{t["chip_bg"]};border:1px solid {t["chip_border"]};border-radius:20px;
  padding:8px 14px;font-size:13px;font-weight:600;display:block;
  text-align:center;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}}
.ref-row{{display:flex;align-items:flex-start;gap:12px;padding:12px 0;border-bottom:1px solid {t["border"]};}}
.coeff-row{{display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid {t["border"]};}}
.cci-badge{{background:{t["chip_bg"]};border:1px solid {t["chip_border"]};border-radius:10px;
  padding:14px 18px;flex:1;min-width:120px;}}
.cci-badges-row{{display:flex;flex-wrap:wrap;gap:10px;margin-bottom:12px;}}
.cb-lbl{{font-size:11px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;
  color:{t["text_secondary"]};margin-bottom:6px;}}
.cb-val{{font-family:"Space Grotesk",sans-serif;font-size:28px;font-weight:700;line-height:1;}}
.cb-sub{{font-size:13px;color:{t["text_secondary"]};margin-top:5px;}}
.plt-flag{{border-radius:8px;padding:12px 16px;font-size:14px;margin-top:10px;line-height:1.6;}}
.step-nav-hint{{font-size:13px;color:{t["text_secondary"]};margin-top:5px;}}

/* ── RESPONSIVE ─────────────────────────────────────────────────────────── */
*,*::before,*::after{{box-sizing:border-box !important;}}
.block-container{{padding:1rem 1rem 3rem 1rem !important;max-width:1400px !important;}}
[data-testid="stAppViewBlockContainer"]{{padding-left:1rem !important;padding-right:1rem !important;}}
.stNumberInput,.stSelectbox,.stTextInput{{width:100% !important;}}
.stNumberInput>div,.stSelectbox>div,.stTextInput>div{{width:100% !important;}}

.icu-nav{{display:flex;align-items:center;justify-content:space-between;padding:14px 0 12px 0;gap:10px;flex-wrap:wrap;}}
.icu-nav-left{{display:flex;align-items:center;gap:12px;flex:1;min-width:0;}}
.icu-nav-right{{display:flex;align-items:center;gap:10px;flex-shrink:0;}}
.nav-title{{font-family:"Space Grotesk",sans-serif;font-size:18px;font-weight:700;color:{t["text_primary"]};white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}}
.nav-sub{{font-size:12px;color:{t["text_secondary"]};letter-spacing:0.06em;text-transform:uppercase;margin-top:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}}
.theme-btn{{width:40px;height:40px;border-radius:50%;cursor:pointer;background:{t["icon_btn_bg"]};border:1.5px solid {t["icon_btn_border"]};display:flex;align-items:center;justify-content:center;font-size:20px;flex-shrink:0;transition:all 0.2s ease;}}
.theme-btn:hover{{background:{t["teal_glow"]};box-shadow:0 0 14px {t["teal_glow"]};transform:rotate(18deg) scale(1.1);}}

.stepper-wrap{{display:flex;align-items:center;gap:0;margin:0 0 24px 0;padding:16px 20px;background:{t["card_bg"]};border:1px solid {t["border"]};border-radius:14px;overflow-x:auto;-webkit-overflow-scrolling:touch;}}
.stepper-wrap::-webkit-scrollbar{{height:0;}}
.step-node{{display:flex;align-items:center;gap:8px;flex-shrink:0;}}
.step-circle{{width:36px;height:36px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-family:"Space Grotesk",sans-serif;font-size:14px;font-weight:700;flex-shrink:0;border:2px solid;transition:all 0.25s;}}
.step-circle.done{{background:{t["teal"]};border-color:{t["teal"]};color:#fff;box-shadow:0 0 12px {t["teal_glow"]};}}
...
.step-label{{font-family:"Space Grotesk",sans-serif;font-size:12px;font-weight:700;letter-spacing:0.06em;text-transform:uppercase;white-space:nowrap;}}
.step-label.done{{color:{t["teal"]};}}
.step-label.active{{color:{t["text_primary"]};}}
.step-label.idle{{color:{t["text_secondary"]};}}
.step-line{{flex:1;height:2px;min-width:20px;margin:0 10px;background:{t["border"]};border-radius:2px;}}
.step-line.done{{background:{t["teal"]};}}

/* Button row alignment */
div[data-testid="stHorizontalBlock"]:has(button[kind="primary"]){{align-items:stretch !important;}}
div[data-testid="stHorizontalBlock"]:has(button[kind="primary"])>div[data-testid="stColumn"]{{display:flex !important;flex-direction:column !important;}}
div[data-testid="stHorizontalBlock"]:has(button[kind="primary"]) .stButton{{flex:1 !important;display:flex !important;flex-direction:column !important;padding:0 !important;margin:0 !important;}}
div[data-testid="stHorizontalBlock"]:has(button[kind="primary"]) .stButton>button{{flex:1 !important;height:100% !important;}}

/* Theme toggle */
.stButton>button[title="Switch to Dark Mode"],.stButton>button[title="Switch to Light Mode"]{{width:42px !important;height:42px !important;border-radius:50% !important;padding:0 !important;font-size:20px !important;line-height:1 !important;background:{t["icon_btn_bg"]} !important;border:1.5px solid {t["icon_btn_border"]} !important;color:{t["teal"]} !important;box-shadow:none !important;letter-spacing:0 !important;display:flex !important;align-items:center !important;justify-content:center !important;margin-left:auto !important;transition:all 0.2s ease !important;}}
.stButton>button[title="Switch to Dark Mode"]:hover,.stButton>button[title="Switch to Light Mode"]:hover{{background:{t["teal_glow"]} !important;box-shadow:0 0 14px {t["teal_glow"]} !important;transform:rotate(18deg) scale(1.1) !important;}}
div[data-testid="stColumn"]:last-child .stButton{{display:flex !important;justify-content:flex-end !important;padding-top:0 !important;margin-top:0 !important;}}
...
@media screen and (max-width:768px){{
  ...
  div[data-testid="stHorizontalBlock"]>div[data-testid="stColumn"]{{width:100% !important;min-width:100% !important;flex:none !important;}}
  div[data-testid="stHorizontalBlock"]:has(button[title="Switch to Dark Mode"]),div[data-testid="stHorizontalBlock"]:has(button[title="Switch to Light Mode"]){{flex-direction:row !important;align-items:center !important;gap:0 !important;}}
  div[data-testid="stHorizontalBlock"]:has(button[title="Switch to Dark Mode"])>div[data-testid="stColumn"],div[data-testid="stHorizontalBlock"]:has(button[title="Switch to Light Mode"])>div[data-testid="stColumn"]{{width:auto !important;min-width:0 !important;flex:1 !important;}}
  div[data-testid="stHorizontalBlock"]:has(button[title="Switch to Dark Mode"])>div[data-testid="stColumn"]:last-child,div[data-testid="stHorizontalBlock"]:has(button[title="Switch to Light Mode"])>div[data-testid="stColumn"]:last-child{{flex:0 0 52px !important;width:52px !important;}}
  .mchip-grid{{grid-template-columns:1fr !important;gap:10px !important;}}
  .mchip{{padding:16px 18px !important;}}
  .mchip .mv{{font-size:24px !important;}}
  .mchip .ml{{font-size:10px !important;}}
  .mchip .ci{{white-space:normal !important;}}
  .icu-stat{{padding:16px 18px !important;}}
  .icu-stat .val{{font-size:28px !important;}}
  .icu-stat .lbl{{font-size:11px !important;}}
  .risk-banner{{padding:16px 18px !important;}}
  .cci-badge{{min-width:unset !important;}}
  .cb-val{{font-size:24px !important;}}
  div[data-testid="stRadio"]>div{{flex-direction:column !important;gap:8px !important;}}
  div[data-testid="stRadio"] label{{width:100% !important;padding:12px 16px !important;}}
  h1{{font-size:24px !important;line-height:1.2 !important;}}
  .feat-chip{{font-size:12px !important;padding:6px 10px !important;}}
  .stCheckbox label{{font-size:14px !important;}}
  .stDataFrame td{{font-size:12px !important;}}
  .stDataFrame th{{font-size:11px !important;}}
  .sec-ttl{{font-size:11px !important;}}
  .step-label{{font-size:11px !important;}}
  .step-circle{{width:30px !important;height:30px !important;font-size:12px !important;}}
  .step-line{{min-width:12px !important;margin:0 6px !important;}}
  .stNumberInput input,.stTextInput input{{font-size:16px !important;padding:12px 14px !important;}}
  [data-baseweb="select"] span{{font-size:15px !important;}}
  .stButton>button:not([title="Switch to Dark Mode"]):not([title="Switch to Light Mode"]){{font-size:14px !important;padding:13px 16px !important;width:100% !important;}}
}}

/* ══ 480px ══ */
@media screen and (max-width:480px){{
  .block-container{{padding:0.5rem 0.5rem 2rem 0.5rem !important;}}
  [data-testid="stAppViewBlockContainer"]{{padding-left:0.5rem !important;padding-right:0.5rem !important;}}
  .icu-badge{{display:none !important;}}
  .nav-title{{font-size:13px !important;}}
  .icu-logo{{width:34px !important;height:34px !important;border-radius:10px !important;}}
  .mchip-grid{{gap:8px !important;}}
  .mchip{{padding:12px 14px !important;}}
  .mchip .mv{{font-size:20px !important;}}
  .mchip .ml{{font-size:9px !important;}}
  .icu-stat .val{{font-size:22px !important;}}
  .cb-val{{font-size:20px !important;}}
  h1{{font-size:20px !important;}}
  .step-label{{font-size:9px !important;letter-spacing:0.01em !important;}}
  .step-circle{{width:26px !important;height:26px !important;font-size:11px !important;}}
  .step-line{{min-width:8px !important;margin:0 4px !important;}}
  .stCheckbox label{{font-size:13px !important;}}
  .stNumberInput input{{font-size:15px !important;}}
  .feat-chip{{font-size:11px !important;padding:5px 8px !important;}}
  .stDataFrame{{overflow-x:auto !important;}}
  .stDataFrame td,.stDataFrame th{{font-size:11px !important;padding:5px 6px !important;}}
}}

/* ══ 320px ══ */
@media screen and (max-width:320px){{
  .block-container{{padding:0.4rem 0.4rem 1.5rem 0.4rem !important;}}
  .mchip .mv{{font-size:18px !important;}}
  .icu-stat .val{{font-size:19px !important;}}
  h1{{font-size:18px !important;}}
  .step-label{{display:none !important;}}
  .icu-logo{{display:none !important;}}
}}
...
@media screen and (min-width:1400px){{
  ...
  .mchip{{padding:24px 26px !important;}}
  .mchip .mv{{font-size:30px !important;}}
  .icu-stat .val{{font-size:38px !important;}}
  .nav-title{{font-size:20px !important;}}
  .nav-sub{{font-size:13px !important;}}
  .mchip-grid{{gap:16px !important;}}
}}
</style>"""
...
# Baseline cumulative hazard H0(t) — Nelson-Aalen estimator from cohort data.
# Day 30 carries the Day 26 value: standard practice when no events were observed
# between Day 26 and Day 30 (step function remains flat until the next event).
BASELINE_HAZARD = {
    1:7.188409e-6,  2:1.462353e-5,  3:1.880582e-5,  4:3.776369e-5,  5:8.204665e-5,
    6:9.924096e-5,  7:1.094765e-4,  8:1.316561e-4,  9:1.448352e-4,  10:1.593160e-4,
    11:1.789804e-4, 12:2.001373e-4, 13:2.001373e-4, 14:2.477519e-4, 15:2.746685e-4,
    16:3.336940e-4, 17:3.666375e-4, 18:4.028481e-4, 19:4.419006e-4, 20:4.419006e-4,
    21:4.912977e-4, 22:5.611107e-4, 24:6.522829e-4, 26:7.556322e-4,
    30:7.556322e-4,  # No new events observed between Day 26–30; step function held constant
}
CCI_CONDITIONS = {
...
CFS_OPTIONS = {
    1:"1: Very Fit", 2:"2: Fit", 3:"3: Managing Well",
    4:"4: Living with Very Mild Frailty", 5:"5: Living with Mild Frailty", 6:"6: Living with Moderate Frailty",
    7:"7: Living with Severe Frailty", 8:"8: Living with Very Severe Frailty", 9:"9: Terminally Ill",
}
RISK_CFG = {
...
def compute(age, ca, inr, mv, cci, cfs, hct, cl):
    ...
    bd = {
        ...
        "ln(INR)":                BETA["log_inr"]         * li,
        "Mechanical Ventilation": BETA["mechanical_vent"] * mv_i,
        "ln(Age)":                BETA["log_age"]         * la,
        "ln(CCI)":                BETA["log_cci"]         * lc,
...
with nav_left:
    st.markdown(f"""
    <div class="icu-nav-left" style="display:flex;align-items:center;gap:14px;padding:6px 0;">
      <div class="icu-logo">
      ...
      <span class="icu-badge" style="margin-left:auto;">Version 2.0 &nbsp;|&nbsp; Sept 2025</span>
    </div>
...
if page == "home":
    ...
    with col_hero:
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));
                    gap:8px;margin-top:18px;margin-bottom:24px;">
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if step == 1:
        ...
        with tl:
            ...
            cfs_v = st.selectbox("Clinical Frailty Scale",
                ["— Select CFS score —"] + [CFS_OPTIONS[i] for i in range(1, 10)],
                index=0, key="cfs_input",
                help="Score ranges from 1 (Very Fit) to 9 (Terminally Ill).")
    ...
    # ════════════════════════════════════════════════════════════════════════
        ...
        {"" if ct2 > 0 else f'<div style="font-size:13px;color:{T["text_secondary"]};padding:6px 12px;background:{T["chip_bg"]};border:1px solid {T["border"]};border-radius:8px;margin-bottom:12px;line-height:1.6;"><strong>ℹ️ CCI = 0:</strong> No comorbidities recorded. The model applies ln(0.5) as the minimum floor — standard Cox PH practice to avoid ln(0) = −∞. This slightly reduces the composite score compared to CCI = 1.</div>'}
        """, unsafe_allow_html=True)
    ...
    # ════════════════════════════════════════════════════════════════════════
        ...
        cfs_opts_list = ["— Select CFS score —"] + [CFS_OPTIONS[i] for i in range(1, 10)]
        cfs_n = cfs_opts_list.index(cfs_r) if cfs_r in cfs_opts_list else 1

        # ── Issue 4: Clinical range warnings ──────────────────────────────
        warnings = []
        if inr_r is not None and inr_r > 10.0:
            warnings.append(f"⚠️ INR {inr_r} is extremely elevated (>10). Verify entry — values this high are rare and may indicate a data entry error.")
        if inr_r is not None and inr_r < 0.5:
            warnings.append(f"⚠️ INR {inr_r} is below the model floor (0.5). The model will use ln(0.5) as the minimum.")
        if hct_r is not None and hct_r < 15.0:
            warnings.append(f"⚠️ Haematocrit {hct_r}% is critically low (<15%). Verify entry — this may indicate a data entry error.")
        if hct_r is not None and hct_r > 60.0:
            warnings.append(f"⚠️ Haematocrit {hct_r}% is above the physiological range (>60%). Verify entry.")
        if cl_r is not None and (cl_r < 80 or cl_r > 130):
            warnings.append(f"⚠️ Chloride {cl_r} mmol/L is outside the plausible clinical range (80–130). Verify entry.")
        if warnings:
            for w in warnings:
                st.warning(w)
        ...
        with chb_r:
            ...
            st.markdown(f"""
            <div style="margin:0 0 8px 0;padding:16px 18px;background:{rc['bg']};
                        border:1px solid {rc['border']};border-radius:12px;">
              <div style="font-size:15px;color:{rc['color']};font-weight:700;
        ...
        with st.expander("Day-by-Day Survival and Mortality Table — All 25 Time Points"):
            ...
            st.markdown(f"""
            <div style="font-size:13px;color:{T["text_secondary"]};padding:8px 0;line-height:1.6;">
              <strong>† Note:</strong> Day 26 was the last observed event in the derivation cohort.
              The Nelson-Aalen baseline hazard step function remains flat between Day 26 and Day 30
              (no new events observed). This is standard practice in Cox PH survival analysis —
              the 30-day estimate is therefore a conservative lower bound for that interval.
            </div>""", unsafe_allow_html=True)
        ...
        <div style="font-family:'Space Grotesk',sans-serif;font-size:14px;font-weight:700;
                    color:{T["teal"]};letter-spacing:0.08em;text-transform:uppercase;
                    margin-bottom:14px;">Download Results</div>
        """, unsafe_allow_html=True)

        # Build full day-by-day table for download
        dl_rows = []
        for d in sorted(BASELINE_HAZARD):
            m_d = mort_at(exp_s, d)
            lo_d, hi_d = mort_ci_95(exp_s, se_comp, d)
            dl_rows.append({
                "ICU Day": d,
                "Survival (%)": f"{100-m_d:.2f}",
                "Mortality (%)": f"{m_d:.2f}",
                "95% CI Lower": f"{lo_d:.2f}",
                "95% CI Upper": f"{hi_d:.2f}",
                "Risk Band": classify(m_d)
            })

        # Build summary report as plain text
        cfs_label = cfs_r if cfs_r != "— Select CFS score —" else "Not recorded"
        cci_selected = [c for c, p in CCI_CONDITIONS.items() if cci_snap.get(c, False)]
        report_lines = [
            "=" * 60,
            "  ICU MORTALITY RISK CALCULATOR — PATIENT REPORT",
            "  Cox Proportional Hazard Model | Version 2.0 | Sept 2025",
            "=" * 60,
            "",
            "PATIENT INPUTS",
            "-" * 40,
            f"  Age:                     {age_r} years",
            f"  Cardiac Arrest:          {'Yes' if ca_n else 'No'}",
            f"  Mechanical Ventilation:  {'Yes' if mv_n else 'No'}",
            f"  Clinical Frailty Scale:  {cfs_label}",
            f"  INR:                     {inr_r}",
            f"  Platelet Count:          {plt_r} x10^9/L",
            f"  Hematocrit:              {hct_r}%",
            f"  Chloride:                {cl_r} mmol/L",
            f"  CCI Score:               {cci_r} (Age pts: {cci_age_r}, Condition pts: {cci_cond_r})",
            f"  CCI Conditions:          {', '.join(cci_selected) if cci_selected else 'None'}",
            "",
            "MODEL OUTPUTS",
            "-" * 40,
            f"  Composite Score:         {comp:+.6f}",
            f"  exp(Composite):          {exp_s:.6f}",
            f"  SE (Composite):          {se_comp:.6f}",
            "",
            "MORTALITY ESTIMATES",
            "-" * 40,
            f"  7-Day Mortality:         {m7:.2f}%  (95% CI: {ci7_lo:.1f}–{ci7_hi:.1f}%)",
            f"  14-Day Mortality:        {m14:.2f}%  (95% CI: {ci14_lo:.1f}–{ci14_hi:.1f}%)",
            f"  30-Day Mortality:        {m30:.2f}%  (95% CI: {ci30_lo:.1f}–{ci30_hi:.1f}%)",
            f"  Risk Classification:     {rl}  ({rc['band']})",
            "",
        ...
        for k, v in bd.items():
            pct = abs(v) / max(sum(abs(x) for x in bd.values()), 0.001) * 100
            report_lines.append(f"  {k:<28} {v:+.6f}  ({pct:.1f}% of score)")
        report_lines += [
            "",
            "DAY-BY-DAY SURVIVAL TABLE",
            "-" * 40,
            f"  {'ICU Day':<10} {'Survival %':<14} {'Mortality %':<14} {'95% CI Lo':<12} {'95% CI Hi':<12} {'Risk Band'}",
        ]
        for row in dl_rows:
            report_lines.append(
                f"  {str(row['ICU Day']):<10} {row['Survival (%)']:<14} {row['Mortality (%)']:<14} "
                f"{row['95% CI Lower']:<12} {row['95% CI Upper']:<12} {row['Risk Band']}"
            )
        report_lines += [
            "",
            "=" * 60,
            "  DISCLAIMER: Decision-support aid for trained clinical",
            "  professionals only. Does not replace multidisciplinary",
            "  team judgement. All outputs are probabilistic estimates.",
            "=" * 60,
        ]
        report_text = "\n".join(report_lines)
        ...
        # Build CSV
        summary_csv_rows = [
            {"Metric": "Age (years)", "Value": age_r},
            {"Metric": "Cardiac Arrest", "Value": "Yes" if ca_n else "No"},
            {"Metric": "Mechanical Ventilation", "Value": "Yes" if mv_n else "No"},
            {"Metric": "CFS Score", "Value": cfs_label},
            {"Metric": "INR", "Value": inr_r},
            {"Metric": "Platelet Count (x10^9/L)", "Value": plt_r},
            {"Metric": "Hematocrit (%)", "Value": hct_r},
            {"Metric": "Chloride (mmol/L)", "Value": cl_r},
            {"Metric": "CCI Score", "Value": cci_r},
            {"Metric": "", "Value": ""},
            {"Metric": "Composite Score", "Value": f"{comp:+.6f}"},
            {"Metric": "7-Day Mortality (%)", "Value": f"{m7:.2f}"},
            {"Metric": "7-Day CI Lower (%)", "Value": f"{ci7_lo:.2f}"},
            {"Metric": "7-Day CI Upper (%)", "Value": f"{ci7_hi:.2f}"},
            {"Metric": "14-Day Mortality (%)", "Value": f"{m14:.2f}"},
            {"Metric": "14-Day CI Lower (%)", "Value": f"{ci14_lo:.2f}"},
            {"Metric": "14-Day CI Upper (%)", "Value": f"{ci14_hi:.2f}"},
            {"Metric": "30-Day Mortality (%)", "Value": f"{m30:.2f}"},
            {"Metric": "30-Day CI Lower (%)", "Value": f"{ci30_lo:.2f}"},
            {"Metric": "30-Day CI Upper (%)", "Value": f"{ci30_hi:.2f}"},
            {"Metric": "Risk Classification", "Value": rl},
        ]
        summary_df = pd.DataFrame(summary_csv_rows)
        daybyDay_df = pd.DataFrame(dl_rows)
        ...
        with dl1:
            st.download_button(
                ...
                data=report_text,
                file_name=f"ICU_Risk_Report_Age{age_r}_{rl.replace(' ','_')}.txt",
                mime="text/plain",
        ...
        with dl2:
            # Combine summary + day table into one CSV with a blank row separator
            import io
            buf = io.StringIO()
            summary_df.to_csv(buf, index=False)
            buf.write("\n")
            daybyDay_df.to_csv(buf, index=False)
            st.download_button(
                label="Download Data Table (.csv)",
                data=buf.getvalue(),
                file_name=f"ICU_Risk_Data_Age{age_r}_{rl.replace(' ','_')}.csv",
                mime="text/csv",
                use_container_width=True,
                key="dl_csv"
            )
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with a1:
        ...
        <div class="icu-card">
          ...
          <ul style="font-size:15px;color:{T["text_secondary"]};line-height:2.1;padding-left:20px;">
            ...
            <li><strong style="color:{T["text_primary"]};">Log floor fix:</strong> CCI=0 uses ln(0.5) instead of ln(0). This is standard practice in Cox PH models — ln(0) is undefined (−∞). A floor of 0.5 is the smallest plausible non-zero value and is consistent with published Cox PH implementations for this variable.</li>
    ...
    with a2:
        ...
        for var, typ, beta, se, note in [
            ("Cardiac Arrest",         "Binary (0/1)",  "+2.7268",  "±0.4120",  "Strongest predictor"),
            ("ln(INR)",                "ln(INR≥0.5)",   "+1.0334",  "±0.2980",  "Coagulopathy"),
            ("Mechanical Ventilation", "Binary (0/1)",  "+0.9076",  "±0.1870",  "Respiratory failure"),
            ("ln(Age)",                "ln(Age≥18)",    "+1.5523",  "±0.3410",  "Age-adjusted risk"),
            ("ln(CCI)",                "ln(CCI≥0.5)",   "+0.8597",  "±0.2030",  "Comorbidity burden"),
            ("CFS Score",              "1–9 scale",     "+0.0598",  "±0.0280",  "Frailty"),
            ("Haematocrit",            "%",             "−0.0583",  "±0.0180",  "Anaemia / perfusion"),
