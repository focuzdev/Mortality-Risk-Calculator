...
/* Force Streamlit's inner wrapper to fill viewport */
...
/* Remove Streamlit's default narrow column cap */
...
@media screen and (max-width:768px){{
  ...
  .icu-badge{{font-size:10px !important;padding:4px 10px !important;}}
  ...
  .step-line{{width:2px !important;height:20px !important;min-width:2px !important;margin:0 0 0 20px !important;}}
  .step-label{{font-size:12px !important;}}
  .step-circle{{width:36px !important;height:36px !important;font-size:13px !important;}}
  /* Cards */
  .icu-card{{padding:16px !important;border-radius:12px !important;}}
  .icu-stat{{padding:14px 16px !important;}}
  .icu-stat .val{{font-size:24px !important;}}
  /* Risk banner */
  .risk-banner{{padding:16px 18px !important;}}
  /* Metric chips */
  .mchip{{padding:14px 16px !important;}}
  .mchip .mv{{font-size:20px !important;}}
  /* Typography */
  h1{{font-size:28px !important;line-height:1.2 !important;}}
  .cci-badge{{min-width:120px !important;padding:10px 14px !important;}}
  .cb-val{{font-size:22px !important;}}
  /* Buttons */
  .stButton>button{{font-size:13px !important;padding:10px 16px !important;}}
  /* Radio buttons stack on mobile */
  div[data-testid="stRadio"]>div{{flex-direction:column !important;gap:6px !important;}}
  /* Feature chips wrap */
  .feat-chip{{font-size:11px !important;padding:5px 10px !important;}}
  /* CCI badges */
  .cci-badge{{flex:0 0 calc(50% - 7px) !important;}}
}}
@media screen and (max-width:480px){{
  /* Small phones */
  .block-container{{padding:0.75rem 0.75rem 2rem 0.75rem !important;}}
  h1{{font-size:24px !important;}}
  .risk-banner{{padding:14px !important;}}
  .mchip .mv{{font-size:18px !important;}}
  .icu-stat .val{{font-size:20px !important;}}
  /* Nav on tiny screens */
  .icu-badge{{display:none !important;}}
  /* Stack metric chips 2x2 grid */
  div[data-testid="stHorizontalBlock"]>div[data-testid="stColumn"]{{min-width:45% !important;}}
  .step-label{{font-size:11px !important;letter-spacing:0.03em !important;}}
}}
@media screen and (min-width:1400px){{
  /* Large desktop — more breathing room */
  .block-container{{padding:2rem 5rem 4rem 5rem !important;}}
  .risk-banner{{padding:28px 36px !important;}}
  .icu-card{{padding:28px !important;}}
}}
</style>"""
...
# ── NAV BAR ───────────────────────────────────────────────────────────────────
col_logo, col_badge, col_theme = st.columns([7, 2, 1])
...
if page == "home":
    ...
    with hl:
        st.markdown(f"""
        <div style="padding:16px 0 22px 0;">
          <div style="font-family:'Space Grotesk',sans-serif;font-size:12px;font-weight:700;
                      letter-spacing:0.16em;text-transform:uppercase;color:{T["teal"]};margin-bottom:16px;">
            CLINICAL DECISION SUPPORT TOOL</div>
          <h1 style="font-family:'Space Grotesk',sans-serif;font-size:46px;font-weight:700;
                     line-height:1.15;margin:0 0 18px 0;">
            Predict ICU Mortality<br><span style="color:{T["teal"]};">With Precision.</span></h1>
          <p style="font-size:16px;color:{T["text_secondary"]};line-height:1.80;max-width:560px;margin-bottom:30px;">
    ...
    stats_html = '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:14px;">'
    for lbl, val, sub in [
        ...
        stats_html += f"""<div class="icu-stat">
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # ════════════════════════════════════════════════════════════════════════
        ...
        # Parse — use index lookup so no dash-character dependency
        ...
        # ── Safety: if session state was lost, send back to step 1 ──────
        critical_missing = [v for v in [age_r, inr_r, hct_r, cl_r, plt_r] if v is None]
        if critical_missing or ca_r == "Not selected" or mv_r == "Not selected" or cfs_r == "— Select CFS score —":
            st.warning("⚠️ Session data was lost or incomplete. Please re-enter patient details.")
            if st.button("← Restart from Step 1", type="primary", key="restart_btn"):
                keep = {"page", "dark_mode"}
        ...
        # ── Metric cards — responsive 2x2 grid on mobile, 4 across on desktop ──
        cards_data = [
        ...
        cards_html = '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:14px;margin-bottom:8px;">'
        for lbl, val, ci, clr, sub, ci_lbl in cards_data:
            ci_html = f'<div style="font-size:10px;margin-top:3px;opacity:0.70;font-family:JetBrains Mono,monospace;">{ci_lbl}: {ci}</div>' if ci else ""
            cards_html += f"""
