...
with nr:
    ...
    pill_bg   = "#0a1f38" if is_dark else "#dce8f0"
    pill_bdr  = "#00c9b1" if is_dark else "#00897b"
    thumb_bg  = "#00c9b1" if is_dark else "#00897b"
    thumb_l   = "calc(100% - 26px)" if is_dark else "2px"
    ...
    .toggle-outer {{
        ...
        gap:10px; padding-top:14px; pointer-events:none;
    }}
    .toggle-label {{
        font-family:'DM Sans',sans-serif; font-size:12px; font-weight:600;
        color:{lbl_clr}; white-space:nowrap; letter-spacing:0.03em;
    }}
    .toggle-pill {{
    ...
    .toggle-thumb {{
        ...
        background:{thumb_bg};
        position:absolute; top:1px; left:{thumb_l};
    ...
    <div class="toggle-outer">
        <span class="toggle-label">{lbl_text}</span>
        <div class="toggle-pill"><div class="toggle-thumb">{t_icon}</div></div>
    </div>
    """, unsafe_allow_html=True)
    if st.button(btn_label, key="theme_btn", type="secondary", use_container_width=True):
