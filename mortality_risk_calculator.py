...
/* ── Hide the internal _about_nav helper button ── */
button[kind="secondary"]:has(p) p {{
  /* target via text when possible */
}}
/* Hide by matching data-testid pattern — hidden nav trigger */
[data-testid="stBaseButton-secondary"][aria-label="_about_nav"],
button[title="About this tool"][kind="secondary"] {{
  display:none !important;
}}
/* Broad fallback: any secondary button whose visible text is the sentinel */
.stButton > button[kind="secondary"] > div > p:empty ~ * {{display:none;}}
...
if page == "home":
    ...
    with col_hero:
        ...
        .cta-row .stButton > button[kind="primary"] {{
          width:100% !important;
        ...
        .about-btn {{
          ...
          box-shadow:0 0 0 0 {T["teal_glow"]};
        }}
        .about-btn:hover {{
          background:{T["teal_glow"]};
          box-shadow:0 4px 18px {T["teal_glow"]};
        ...
        with b2:
            # Render a polished HTML anchor-style button; use a hidden Streamlit button
            # behind it (triggered via JS) so session_state navigation still works.
            st.markdown(f"""
            <div style="padding-top:2px;">
              <button class="about-btn" onclick="
                (function(){{
                  var btns=window.parent.document.querySelectorAll('button[kind=\\'secondary\\']');
                  for(var b of btns){{if(b.innerText.trim()==='_about_nav'){{b.click();return;}}}}
                }})()
            ...
            if st.button("_about_nav", key="about_nav_hidden",
