...
/* ── Theme pill toggle (rendered via st.columns + st.button) ── */
.theme-track-static {{
  width:38px;height:20px;border-radius:999px;
  background:{_pill_track};position:relative;flex-shrink:0;
  pointer-events:none;
}}
.theme-knob-static {{
...
/* Style the nav-toggle st.button as an invisible overlay */
...
# ── NAV BAR — uses st.columns so Streamlit renders it natively (no onclick/JS) ──
is_dark = st.session_state.dark_mode

_knob_x   = "26px" if is_dark else "2px"
...
nav_logo, nav_title_col, nav_spacer, nav_toggle = st.columns([0.07, 0.72, 0.13, 0.08])
...
with nav_spacer:
    st.markdown(f"""
    <div style="display:flex;align-items:center;height:100%;padding-top:8px;justify-content:flex-end;">
      <span class="icu-badge">Version 2.0 &nbsp;|&nbsp; Sept 2025</span>
    </div>""", unsafe_allow_html=True)

with nav_toggle:
    # Real Streamlit button — CSS in build_css makes it look like a pill toggle
    st.markdown(f"""
    <div style="display:flex;align-items:center;gap:6px;padding-top:6px;justify-content:flex-end;">
      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" style="opacity:{_moon_op};flex-shrink:0;">
        <path d="M21 12.79A9 9 0 1 1 11.21 3a7 7 0 0 0 9.79 9.79z" fill="{T['teal']}"/>
      </svg>
      <div class="theme-track-static">
        <div class="theme-knob-static" style="transform:translateX({_knob_x});"></div>
      </div>
      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" style="opacity:{_sun_op};flex-shrink:0;">
    ...
    if st.button("", key="theme_btn", help=_toggle_label, use_container_width=True):
